...
public abstract class RecalculateTaskScheduleAlgorithm extends AlgorithmBase {
  ...
  private final SortedMap<Integer, List<TaskDependency>> myDistance2dependencyList =
    new TreeMap<Integer, List<TaskDependency>>();
  ...
  private void fulfilConstraints(TaskDependency dependency) throws TaskDependencyException {
    ...
    if (depsAsDependant.length > 0) {
      ...
      GanttCalendar latestStart =
        startLaterVariations.size() >= 0 ? startLaterVariations.get(startLaterVariations.size() - 1)
                                         : null;
  ...
  private void modifyTaskStart(Task task, GanttCalendar newStart) {
    ...
    WorkingUnitCounter counter =
      new WorkingUnitCounter(task.getManager().getCalendar(), task.getDuration().getTimeUnit());
    TimeDuration shift;
