 ...
 */
package net.sourceforge.ganttproject.ui.gui.tableView;
...
import net.sourceforge.ganttproject.ui.gui.AbstractTableAndActionsComponent;
import net.sourceforge.ganttproject.ui.gui.AbstractTableAndActionsComponent.SelectionListener;
import net.sourceforge.ganttproject.ui.gui.EditableList;
import net.sourceforge.ganttproject.ui.gui.ListAndFieldsPanel;
import net.sourceforge.ganttproject.ui.gui.UIUtil;
import net.sourceforge.ganttproject.ui.gui.options.OptionsPageBuilder;
import net.sourceforge.ganttproject.ui.gui.options.model.CustomPropertyDefaultValueAdapter;
...
public class ColumnManagerPanel {
  ...
  public Component createComponent() {
    ...
    List<CustomPropertyDefinition> defs = new ArrayList<>();
    createDefaultFieldDefinitions(myVisibleFields, myManager.getDefinitions(), defs);
    final Set<CustomPropertyDefinition> standardDefs = new HashSet<>(defs);
    defs.addAll(myManager.getDefinitions());
    final EditableList<CustomPropertyDefinition> props = new EditableList<>(defs, emptyList) {
      @Override
      ...
      public void selectionChanged(List<CustomPropertyDefinition> selection) {
        ...
        if (!isShow) {
          for (int i = 0; i < myVisibleFields.getSize(); i++) {
            ...
            if (it.isVisible()) {
              boolean isSelected = !Collections2
                .filter(selection, def -> Objects.equal(def.getName(), it.getName()))
                .isEmpty();
              if (!isSelected) {
                setEnabled(true);
                return;
    ...
    ChangeValueListener defaultValuePanelEnabler = event -> setDefaultValuePanelEnabled(myEnableDefaultValueOption.isChecked());
    ...
    ListAndFieldsPanel<CustomPropertyDefinition> listAndFields = new ListAndFieldsPanel<>(
      props, myFields);

    Function<List<CustomPropertyDefinition>, Boolean> isDeleteEnabled = input -> {
    ...
    props.getTableAndActions().addSelectionListener(selection -> {
      if (selection.size() != 1) {
  ...
  private void createDefaultFieldDefinitions(ColumnList tableHeader,
    ...
    LinkedHashMap<String, Column> name2column = new LinkedHashMap<>();
  ...
  private JComponent getFieldsComponent() {
    ...
    GPOptionGroup defaultValueGroup = new GPOptionGroup("customPropertyDialog.defaultValue", myEnableDefaultValueOption);
