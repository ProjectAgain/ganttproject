...
public class TTFontCache {
  ...
  public void registerDirectory(String path) {
    ...
    if (dir.exists() && dir.isDirectory()) {
      ...
      log.info("directory " + path + " is not readable");
    }
  }
  ...
  private void registerFonts(File dir) {
    ...
    for (File f : files) {
      ...
      try {
        ...
        log.info("Failed to register font from " + f.getAbsolutePath(), e);
      }
    }
  }
  ...
  private void registerFontFile(final File fontFile) throws FontFormatException,
    ...
    log.debug("Trying font file: " + fontFile.getAbsolutePath());
    ...
    log.debug("registering font: " + family);
    if (awtSupplier == null) {
  ...
  public com.itextpdf.text.Font getFont(String family, String charset, int style, float size) {
    ...
    if (result == null) {
      ...
      if (bf != null) {
        ...
        log.error("Font with family={} not found. Also tried fallback font", family);
      }
    }
    return result;
  ...
  public FontMapper getFontMapper(final FontSubstitutionModel substitutions, final String charset) {
    return new FontMapper() {
      ...
      public BaseFont awtToPdf(Font awtFont) {
        ...
        if (result == null) {
          log.error("Font with family={} not found. Also tried family={} and fallback font", awtFont.getFamily(), family );
        }
        return result;
      }
