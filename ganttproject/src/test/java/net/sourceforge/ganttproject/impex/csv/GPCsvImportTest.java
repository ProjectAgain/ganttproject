...
public class GPCsvImportTest {
    ...
    public void testCustomFields() throws Exception {
        String header1 = "Field1," + buildTaskHeader(
        ...
        for (Pair<SpreadsheetFormat, Supplier<InputStream>> pair: createPairs(header1, data1)) {
            ...
            Map<String, Task> taskMap =
                doTestImportAssignments(pair.second(), pair.first(), builder, taskManager, null, null);
            Task t1 = taskMap.get("t1");
            assertNotNull(t1);

            CustomPropertyDefinition def1 =
                taskManager.getCustomPropertyManager().getCustomPropertyDefinition("Field1");
            CustomPropertyDefinition def2 =
                taskManager.getCustomPropertyManager().getCustomPropertyDefinition("Field2");
            assertNotNull(def1);
            assertNotNull(def2);
    ...
    public void testDependencies() throws Exception {
        ...
        for (Pair<SpreadsheetFormat, Supplier<InputStream>> pair: createPairs(
    ...
    public void testEmptyDuration() throws Exception {
        ...
        GanttCSVOpen importer =
            new GanttCSVOpen(createSupplier(Joiner.on('\n').join(header1, data1).getBytes(Charsets.UTF_8)),
    ...
    public void testImportAssignmentsColumn() throws Exception {
        ...
        Map<String, HumanResource> resourceMap =
            Maps.uniqueIndex(resourceManager.getResources(), new Function<HumanResource, String>() {
                @Override
        ...
        assertEquals(
            100f, taskMap.get("t2").getAssignmentCollection().getAssignment(resourceMap.get("Jack")).getLoad());
    ...
    public void testImportResourceRole() throws Exception {
        ...
        String header2 = buildResourceHeader(ResourceRecords.ResourceFields.NAME, ResourceRecords.ResourceFields.ID,
                                             ResourceRecords.ResourceFields.ROLE
        );
    ...
    public void testImportResourcesColumn() throws Exception {
        ...
        for (Pair<SpreadsheetFormat, Supplier<InputStream>> pair: createPairs(header1, data1, "", header2, data2)) {
            ...
            HumanResourceManager resourceManager = builder.getResourceManager();
            Map<String, Task> taskMap =
                doTestImportAssignments(pair.second(), pair.first(), builder, null, resourceManager,
                                        new RoleManagerImpl()
                );
            Task t1 = taskMap.get("t1");
            assertNotNull(t1);
    ...
    public void testUseEndDateInsteadOfDuration() throws Exception {
        ...
        for (Pair<SpreadsheetFormat, Supplier<InputStream>> pair: createPairs(header1, data1)) {
            ...
            Map<String, Task> taskMap = doTestImportAssignments(pair.second(), pair.first(), builder, null, null, null);
            assertEquals(
                4.0f, taskMap.get("t1").getDuration().getLength(builder.getTimeUnitStack().getDefaultTimeUnit()));
        }
    }
    ...
    private Map<String, Task> doTestImportAssignments(
        Supplier<InputStream> supplier, SpreadsheetFormat format, TaskManagerBuilder builder,
        TaskManager taskManager, HumanResourceManager resourceManager, RoleManager roleManager
    ) throws IOException {
        if (taskManager == null) {
            taskManager = builder.build();
        }
        GanttCSVOpen importer =
            new GanttCSVOpen(supplier, format, taskManager, resourceManager, roleManager, builder.getTimeUnitStack());
        importer.load();
